...
(async () => {
  ...
  for (const r of repos) {
    ...
    for (const u of rawReadmeUrls) {
      try {
        ...
        if (res.ok) { readmeText = await res.text(); break; }
      } catch (e) {}
    }
    ...
    if (readmeText) {
      ...
      if (imgUrl) {
        ...
        if (imgUrl.startsWith('./') || imgUrl.startsWith('../') || imgUrl.startsWith('images/') || !/^https?:\/\//i.test(imgUrl)) {
        ...
        const localName = `${r.name.replace(/[^a-z0-9\-]/ig,'_')}${ext}`;
  ...
  const out = template.replace('<!-- REPO_CARDS -->', cardHtml)
